...
- 【颜文字】不要默认加。只有人设本身爱撒娇、爱卖萌、聊天里本来就会用颜文字时，才可在全文点缀最多 2～3 个（不是 emoji），例如 (≧▽≦) (´∀｀)♡ (づ￣3￣)づ。高冷、克制、正经、寡言、阴郁、禁欲等口吻一律不加。禁止连串、禁止每段都加。
- 【落款】signature 要有感情，不要只用证件全名。恋人可写「爱你的××」「最爱你的××」「永远偏心你的××」；可用小名、昵称或名字里更亲的那一截，不必全名。非恋人不要硬套「爱你的」，可贴合关系写克制落款。
...
export async function generateQixiConfessionLetter(params: {
  ...
  const userTask = `请根据下列材料写七夕告白信。${
    lovers
      ...
      : '落款可贴合你们当前关系，不要硬套恋人腔。'
  } 信的最后一段只写对未来的期望，不要写马上见面、开门、在门口等着这类线下约定。颜文字最多 2～3 个，人设不合适就一个都不要加，不要用 emoji。\n\n${contextParts.join('\n\n')}`
