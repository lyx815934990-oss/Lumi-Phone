...
export function buildCrossChannelStoryTimeSyncRule(params: {
  ...
  return (
    ...
    `- 下方「未总结·私聊」为记忆游标之后的线上原文：与「现在」**同日**为近端须承接；**跨日更早**标成往事——角色已知，但**禁止**写成此刻刚聊/即将再走。已写入长期记忆的内容见「已总结·长期记忆」。\n` +
...
export function buildOfflineOnlineSpatialContinuityRule(params: {
  ...
  const lines = [
    ...
    ...(now
      ...
      : []),
    `- 「未总结·私聊」含**同日近端**（约 ${countLabel} 条；记忆游标之后）：角色**已知**；本轮线下须承接。跨日更早标为往事：可回溯，**禁止**当成本轮刚聊。已总结内容见长期记忆。`,
