...
export function buildCrossChannelStoryTimeSyncRule(params: {
  ...
  return (
    ...
    `- 下方「未总结·私聊」按故事「现在」拆成**近端 / 往事**（同日或其后为近端；跨日更早为往事仍注入且角色已知，但**禁止**写成此刻刚聊）。已写入记忆库的摘要见「已总结·长期记忆」。\n` +
    `- 若每条仍带方括号前缀：\`[剧情 …｜系统 …]\` 以剧情侧理解故事先后；仅 \`[系统 …·落库]\` 为设备真实发送/落库钟点，**不是**故事内剧情时刻；**禁止**把 10:20 等前缀误读成故事清晨、另一天或线下已过去的时段。\n` +
