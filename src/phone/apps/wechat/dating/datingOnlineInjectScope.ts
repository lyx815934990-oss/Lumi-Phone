...
export function buildCrossChannelStoryTimeSyncRule(params: {
  ...
  return (
    ...
    `- 本块已剔除剧情时间早于「现在」/上一轮线下的旧气泡。若长期记忆或时间轴仍提及更早的「要离开/分别」等，那是**已发生往事**，**禁止**写成此刻即将再走、正在分别或尚未归来。\n` +
...
export function buildOfflineOnlineSpatialContinuityRule(params: {
  ...
  const lines = [
    ...
    ...(now
      ...
      : []),
    `- 「尚未总结·私聊」含**上一轮线下 AI 之后、且不早于故事「现在」**的 ${countLabel} 条微信（另有近端固定原文窗时一并服从）：本轮线下须视为发生在该段聊天**之后**，与「最近剧情」**末尾**共同构成开场锚点（二者冲突时：**晚于线下的线上事实**优先纠偏当场状态）。更早「要离开/分别」等未再注入的内容视为往事，禁止当成本轮即将发生。`,
