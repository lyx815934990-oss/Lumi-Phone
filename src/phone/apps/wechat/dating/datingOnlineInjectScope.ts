...
export function buildCrossChannelStoryTimeSyncRule(params: {
  ...
  return (
    ...
    `- 下方「未总结·私聊」为该角色**会话近端线上原文**（线下为承接**不按记忆游标砍掉**）：与「现在」**同日**为近端须承接；**跨日更早**标成往事——角色已知，但**禁止**写成此刻刚聊/即将再走。已写入记忆库的摘要见「已总结·长期记忆」。\n` +
...
export function formatDatingOnlineInjectScopeFooter(meta: DatingOnlineInjectScopeMeta): string {
  ...
  return (
    `（↑ 线上私聊原文（会话近端；不因记忆游标漏注；自${anchor}相关时段起参考）；近端约 ${meta.privateMessageCount} 条（同日或相关时段）${span}；` +
    `${timeNote}。上方若另有「未总结·往事私聊」亦是角色已知，仅禁止当此刻；已总结摘要见长期记忆块。）`
  )
}
...
export function buildOfflineOnlineSpatialContinuityRule(params: {
  ...
  const lines = [
    ...
    ...(now
      ...
      : []),
    `- 「未总结·私聊」含**同日近端**（约 ${countLabel} 条；会话原文，**不因记忆游标为空**）：角色**已知**；本轮线下须承接。跨日更早标为往事：可回溯，**禁止**当成本轮刚聊。已总结摘要见长期记忆。`,
