...
const DEFAULT_STYLE_PROMPT = [
  ...
  '轻喜剧点睛：角色炸毛/嘴硬等可见反应后，可用半句旁白轻吐槽收口（如「咦，占有欲真强……」）；用普通旁白，禁止塞进双星号内心 OS；每轮约 1～3 处，禁句句吐槽。',
  '严禁：景物开场长铺、文学腔比喻、情绪标签排比、网文/八股抒情、文末无意义升华、油腻霸总土味。',
  '历史只取事实：最近剧情禁止当文风范本；上一轮亲密同拍/省略号对白模板禁止洗稿；旧稿里的近乎/狼狈/执拗/耳朵红得像滴血等一律当反例，本轮按白描重写。',
  '文末禁：禁止「这一刻你却觉得秋风没那么冷了」「世界忽然安静」「独属于XX的喧嚣里你觉得……」等天气/城市鸡汤收束；末句须是动作或对白。',
  '环境至多一句且须立刻服务动作；禁连续景物/光线/柏油路/楼道氛围堆叠；禁「心口因为某种……而跳得急」这类空感受句。',
  '禁神经生理夸张：禁止「耳朵红得要滴血」「胸腔里的心脏」「心跳撞击肋骨」等解剖腔/科幻级身体描写；脸红心跳用短句直说即可。',
].join(' ')
...
export function buildDatingStyleSystemAppend(gen?: NarrativeGenOptions): string {
  ...
  parts.push(
    [
      ...
      '✗ 神经腔：耳朵红得要滴血；胸腔里的心脏；心跳一下下撞击肋骨——读着累、像硬堆生理感。',
      '✓ 白描：先动作或对白；环境最多一句且立刻影响下一步；心理用短 OS / 可见神态，不写散文抒情。',
      '✓ 神态：耳根红了 / 心跳很快 / 呼吸乱了——一句说清，禁止解剖课口吻与科幻级夸张。',
      '✓ 旁白轻吐槽：先写出炸毛/嘴硬等可见反应，再另起半句场外点评（「咦，占有欲真强……」）；普通旁白，勿用 **OS**；每轮点睛几处即可。',
    ].join('\n'),
  )
  return `\n\n${parts.join('\n\n')}`
}
