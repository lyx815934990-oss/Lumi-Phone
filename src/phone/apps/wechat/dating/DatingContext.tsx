...
const STYLE_HINT =
  '旁白直写；对白只能用双引号"..."；内心OS：**仅**用一对英文半角 ** 包裹**可读心思（与界面渲染一致）；**单条 OS 宜 2～4 句、合计约 45～120 汉字（不少于 40 汉字）**，写潜台词与未说出口的那句，**禁止**十来字敷衍；**禁止**星号内只有「我……」「我…」占位；**禁止**在 ** 外单独缀一行「我……」；**禁止** OS 里骂/幼化 {{user}}、量胆子、狠话贬低身板、或嘲讽对方狠话（「傻逼」「胆子越来越大了」「你那小身板可以吗」「到时候你可别先认输」）——须遵守 `<CharUserEqualityOverride>` / `<NoCourageJudgment>` / `<NoThreatBelittlement>` / 内心 OS 平等铁律；上帝视角时旁白用他/她写约会对象与 NPC，他人心念/视线指向玩家须用「你」勿用身份卡姓名；OS 内「我」仍指约会对象且须语义连贯，勿在 OS 里写第三人称评价串戏。' +
