...
- **默认同日（硬）**：接续同一场戏/同一地点的下一段时，若正文**没有**明示跨日（第二天、次日、明天、隔夜到天亮、写下一日日期等），story_day **必须**与【剧情时间锚点】**同一公历日**，只允许时刻略往后；**禁止**为了「推进」把 9月9日 写成 9月10日/11日。
...
export async function buildStoryTimelineCalendarContextBlock(params: {
  ...
  if (anchor) {
    ...
    lines.push(
      '若正文未写出明确公历年月日：story_day/story_time **必须**采用上述锚点的**同日**（时刻可略后）；禁止臆造更早年份，也禁止无跨日描写就跳到次日。',
    )
  } else {
    lines.push(
