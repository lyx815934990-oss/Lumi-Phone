...
- **默认同日（硬）**：接续同一场戏/同一地点的下一段时，若正文与玩家输入/导演指令**都没有**明示跨日（第二天、次日、明天、到了次日、隔夜到天亮、写下一日日期等），story_day **必须**与【剧情时间锚点】**同一公历日**，只允许时刻略往后；**禁止**为了「推进」把 9月9日 写成 9月10日/11日。
- **有跨日证据则必须换日（硬）**：玩家输入/导演指令或正文明写「次日/明天/第二天/到了次日」等时，story_day **必须**写成锚点日的**次日**（后天则+2），**禁止**仍写锚点同一公历日；同日也**禁止**把 story_time 写成早于锚点末尾时刻（如锚点已是 20:40，不得写成 15:00）。
...
【剧情时间·接续铁律（高于自行编造年份）】
- 接续同一场戏时：story_day **默认=【剧情时间锚点】同一公历日**，时刻可略往后；**禁止**正文未写跨日却把日期改成次日/后日（例：锚点 9月9日 17:01，续写仍在琴房→仍须 9月9日，不得写成 9月10日）。
- **有跨日证据必须换日**：导演指令/玩家输入或正文明写「次日/明天/第二天/到了次日」等时，story_day **必须**为锚点次日（或后天等），**禁止**正文写「次日」却仍标同一公历日；同日续写**禁止**时刻早于锚点末尾（例：锚点 20:40 → 不得标 15:00）。
- 仅当正文明示跨日（第二天/次日/明天/隔夜到天亮/写下一日日期等）或用户已把故事「现在」跳到更晚公历日时，才可填写更晚的 story_day。
...
export async function buildStoryTimelineCalendarContextBlock(params: {
  ...
  if (anchor) {
    ...
    lines.push(
      '若正文未写出明确公历年月日：story_day/story_time **必须**采用上述锚点的**同日**（时刻可略后）；禁止臆造更早年份，也禁止无跨日描写就跳到次日。若玩家输入/导演指令或正文明写「次日/明天」等，story_day **必须**写成锚点次日，禁止仍写同日。',
    )
  } else {
    lines.push(
