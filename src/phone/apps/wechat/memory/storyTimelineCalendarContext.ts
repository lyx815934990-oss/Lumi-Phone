...
- **默认同日（硬）**：接续同一场戏/同一地点的下一段时，若正文与玩家输入/导演指令**都没有**明示**场景已经切到**次日（到了次日、隔夜到天亮、通宵到清晨、写下一日日期且已发生等），story_day **必须**与【剧情时间锚点】**同一公历日**，只允许时刻略往后；**禁止**为了「推进」把 9月9日 写成 9月10日/11日。
- **约好明天 ≠ 已到明天（硬）**：对白/OS 里「明天晚上见」「次日再聊」「下周见」只是**计划**，本场戏仍在当日；story_day **禁止**因此改成次日。只有写明已经历了跨日（如「到了次日清晨」「隔夜到天亮」）才可换日。
- **有跨日证据则必须换日（硬）**：玩家输入/导演指令或正文明写**已发生**的切日时，story_day **必须**写成锚点日的**次日**（后天则+2），**禁止**仍写锚点同一公历日；同日也**禁止**把 story_time 写成早于锚点末尾时刻（如锚点已是 20:40，不得写成 15:00）。
- **正文未明示公历**：若材料/正文没有写出明确年月日（或只有「傍晚」等模糊词），story_day / story_time **必须**采用下方【剧情时间锚点】的**同日**（时刻可略后）；**禁止**臆造更早年份，也**禁止**无依据跳到次日。
- **跨时段剧情**：仅当正文明确跨越时间（闪回、多日旅行、从清晨写到次日凌晨且写明跨日）时，才可填写晚于锚点日的 story_day，或填写 story_day_end、story_time_end；展示形如「2026年5月1日 08:00 - 2026年6月29日 18:00」。**同日**跨度只写 story_time + story_time_end（story_day_end 省略或与 story_day 相同）；**禁止**无跨日描写却把 story_day_end 写成次日。
...
/** 约会正文生成用：接续上一回合锚点，禁止公历倒流与无依据跨日 */
export const STORY_TIMELINE_CALENDAR_CHRONOLOGY_RULES = `
【剧情时间·接续铁律（高于自行编造年份）】
- 接续同一场戏时：story_day **默认=【剧情时间锚点】同一公历日**，时刻可略往后；**禁止**正文未写**已发生的切日**却把日期改成次日/后日（例：锚点 9月9日 17:01，续写仍在琴房→仍须 9月9日，不得写成 9月10日）。
- **约好明天 ≠ 已到明天**：对白/OS「明天晚上见」「次日再聊」「下周见」只是计划，本场戏仍在当日；**禁止**因此把 story_day 写成次日。
- **有已切日证据才可换日**：正文/导演指令明写**已经历**的切日（到了次日、隔夜到天亮、通宵到清晨、写下一日日期且已发生等）时，story_day 才可写成锚点次日（或后天等）；同日续写**禁止**时刻早于锚点末尾（例：锚点 20:40 → 不得标 15:00）。
- **禁止**无回忆/闪回/插叙铺垫却把公历写成更早年份（例：锚点已是 2026 年却写 2024）。
...
export async function buildStoryTimelineCalendarContextBlock(params: {
  ...
  if (anchor) {
    ...
    lines.push(
      '若正文未写出明确公历年月日：story_day/story_time **必须**采用上述锚点的**同日**（时刻可略后）；禁止臆造更早年份，也禁止无**已切日**描写就跳到次日。「约好明天/次日见」只是计划，禁止因此写成次日；仅当正文写明已到次日/隔夜到天亮等，才可换日。',
    )
  } else {
    lines.push(
