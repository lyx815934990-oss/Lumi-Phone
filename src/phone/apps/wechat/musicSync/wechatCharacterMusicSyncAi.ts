...
export function buildCharacterMusicSyncInviteStateBias(
  ...
  if (!latest) {
    return `[一起听·状态] 当前未与该用户处于共听会话（用户可能已暂停/关掉播放）。若要一起听，须发 \`共听邀请\` 邀约卡并等用户点击接受；**禁止** \`点歌\` / \`跳进度\` / \`切下一首\` / \`切上一首\` 直接点播或切歌；**不要**假装还在共听或继续主动聊刚才的歌。`
  }
  ...
  if (latest.invite.userResponded === 'accepted') {
    return `[一起听·状态] 用户曾接受共听（《${title}》），但当前已无活跃共听（可能已暂停很久或关掉播放）。勿假装还在一起听、勿主动接着聊那首歌；再次一起听须重新发 \`共听邀请\`，不可 \`点歌\` 直播。`
  }
  return ''
}
...
- **对白风格**：优先贴合用户当下情绪与话题；共听歌词别太当回事——可偶尔轻带氛围，勿深挖/吃飞醋/歌词研讨会。**禁止**报时间码、进度、「我刚帮你切了歌」等无意义报幕。
