...
- 须符合人设分寸：卸下对外伪装 ≠ 放大阴暗面。傲娇可写别扭的懊悔，高冷可写克制的想念；允许浪漫与亲密回忆。
- **健康三观 · 禁病态占有（硬）**：全文须是**平等、珍惜、可沟通**的在意。**禁止**极端绑架式/囚禁式措辞及同义变体——如「牢牢地锁住」「牢牢锁住」「一秒也不想离开」「只想看着你」「离不开你」「不能没有你」「没有你就活不了」「你只能是我的」「谁来都撬不走」「占为己有」「不许离开」「只许看着我」等。可写：喜欢靠近、在意、想照顾、想一起好好过、会吃醋但尊重对方空间、希望对方更好。双方是独立个体，不是谁拴死谁。
- 禁止病态偏执、监视控制、标记/囚禁式占有、恐吓威胁。除非人设与近期剧情明确导向，否则不要主动写成病娇、猎奇或过度色情的占有戏；亲密描写点到为止，更侧重心情与相处细节。
...
export async function generateSubconsciousDiaryEntry(params: {
  ...
  const userTask = `请根据下列近期上下文，书写一篇全新的私人日记（语气自然、贴合人设；健康三观；勿写「锁住/离不开/一秒也不想离开/只想看着你」等病态占有独白）。
