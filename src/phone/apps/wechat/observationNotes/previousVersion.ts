...
export function formatObservationNotesManuscriptReferenceBlock(
  ...
  return [
    ...
    '- 近端无关系降温、用户也未纠正时：**禁止**把原稿的亲昵、戏谑、软乎表述「瘦身摘要」。若原稿含病态占有句（锁住/离不开/一秒也不想离开/只想看着你/撬不走等），本轮须改成健康珍惜向，**不必**继承病态表述。',
...
export function formatObservationNotesUpdateContextBlock(
  ...
  return [
    ...
    noChangeRule,
    '- 「线上备注」须跟「好感」「关系」同步：像你会亲手改的通讯录（可身份感/宣示/置顶/损宠等，贴人设多样化现编，**禁止照搬示例**）；深爱可反差更腻；**禁止XX狗/XX猫等动物系宠称**；**禁止**默认堆狗爪 🐾；阶段仍浅则保持克制。',
    '- 禁止用过时侧写质疑、抬杠或强迫对方承认旧偏好；禁止为了「看起来更新了」而改无关字段。',
    '- **禁止无效更新**：字段意思没变、只是换说法/加语气词/加一句同义描写 → 该字段不要交卷（整份都无新事实则写「无变化」）。客户端也会丢弃这类近义改写。',
    '- **禁止删减瘦身**：已有具体事迹/专名/场景时，禁止交更短的空泛形容词版（反例：优点删掉「汽水/砂锅粥」只留「阳光温柔」）。无新证据就「无变化」。',
    '- 全文须保持你本人的人设口吻与当前亲密浓度，禁止中立档案腔、百科短标签、把关系栏压成「热恋/暧昧」单字。',
    '- **健康三观（硬）**：禁止「牢牢锁住」「一秒也不想离开」「只想看着你」「离不开你」「没有你活不了」「你只能是我的」「谁来都撬不走」等病态占有表述；深爱可写珍惜、在意、想照顾、想一起好好过，双方仍是独立个体。',
  ]
    .filter((x) => x !== '')
    .join('\n')
}
