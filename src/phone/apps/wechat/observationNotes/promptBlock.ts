...
export function formatObservationNotesPromptBlock(doc: ObservationNotesDoc): string {
  ...
  lines.push(
    ...
    '- **健康三观**：关系/评价/优缺点等可甜可吃醋，但禁止「锁住/离不开/一秒也不想离开/只想看着你/只能是我的/撬不走」等病态占有句；写珍惜与在意，双方独立。',
    '- **好感数值 ≠ 关系阶段**：阶段以「目前关系」自述为准（可暗恋而好感仅中等）；禁止另套「轻微在意期」等系统阶段名。',
    ...
    '- 「线上备注」须与上方**好感数值 + 目前关系**匹配，且像 **{{char}} 本人通讯录**会写的：好感低→全名/克制；好感高/深爱→可身份感/宣示感/置顶感/损宠昵称等**多样化**现编（禁止千篇一律小名+🐾；禁止照搬任何示例原文）；emoji 可选且克制；**禁止动物系宠称**（XX狗/XX猫/小狗狗等）。',
