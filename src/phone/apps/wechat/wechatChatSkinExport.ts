...
export function buildWeChatChatSkinAiPrompt(): string {
  return [
    ...
    '- 顶栏：--wx-chat-header-bg / --wx-chat-header-bg-image / --wx-chat-header-text / --wx-chat-header-border',
    '- 输入栏：--wx-chat-input-bar-*（含 bg-image）/ --wx-chat-input-shell-* / --wx-chat-input-btn-color',
    '- Lumi 特殊消息变量：',
    '  - 红包 --wx-special-rp-*',
    '  - 转账 --wx-special-tf-*',
    '  - 语音 --wx-special-voice-*',
